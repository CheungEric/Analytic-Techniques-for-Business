...
def answer_two():
    ...
    for degree in range(10):
        ...
        r2_train[degree] = r2_score(y_train, reg.predict(X_tr_))
        r2_test[degree] = r2_score(y_test, reg.predict(X_tst_))      
...
def answer_four():
    ...
    X_trn = X_train.reshape(-1,1)
    X_tst = X_test.reshape(-1,1)
    X_trn_poly = poly.fit_transform(X_trn)
    X_tst_poly = poly.fit_transform(X_tst)
    linreg = LinearRegression().fit(X_trn_poly, y_train)
    linlasso = Lasso(alpha=0.01, max_iter = 10000).fit(X_trn_poly, y_train)
    linreg_test_score = linreg.score(X_tst_poly, y_test)
    lasso_test_score = linlasso.score(X_tst_poly, y_test)
